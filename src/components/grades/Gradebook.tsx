...
const Gradebook: React.FC<GradebookProps> = ({ classId }) => {
  ...
  useEffect(() => {
    if (classId) {
      ...
      const classAssessments = allAssessments.filter(assessment => assessment.classId === classId);
      setAssessments(classAssessments);
      
      // Filter grades directly
      const assessmentIds = classAssessments.map(assessment => assessment.id);
      const classGrades = allGrades.filter(grade => assessmentIds.includes(grade.assessmentId));
      ...
      const gradebookRows: StudentGradeRow[] = students.map(student => {
        ...
        const gradesMap: { [assessmentId: string]: { value: string | number; comments?: string } } = {};
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `${selectedClass?.name || 'class'}_gradebook.csv`);
    ...
    return (
      <GlassCard className="p-6 text-white text-center">
        ...
        <p className="text-white/70">
          Please create assessments for {selectedClass?.name || 'this class'} first
        </p>
      </GlassCard>
    );
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-medium text-white">
            Gradebook
            {selectedClass && (
              <span className="ml-2 text-white/70">for {selectedClass.name}</span>
            )}
          </h3>
            ...
            <Table className="text-white w-full">
              <TableHeader>
                <TableRow className="hover:bg-white/5 border-white/20">
                  <TableHead className="text-white/70 sticky left-0 bg-gray-900">Student</TableHead>
                  {filteredAssessments.map((assessment) => (
                    <TableHead key={assessment.id} className="text-white/70 min-w-[120px]">
                      <div>{assessment.title}</div>
                      <div className="text-xs text-white/60">{assessment.type}</div>
                      <div className="text-xs text-white/60">{new Date(assessment.date).toLocaleDateString()}</div>
                    </TableHead>
                  ))}
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredGradebook.map((row) => (
                  <TableRow key={row.studentId} className="hover:bg-white/5 border-white/10">
                    ...
                    {filteredAssessments.map((assessment) => {
                      const grade = row.grades[assessment.id];
                      return (
                        <TableCell key={`${row.studentId}-${assessment.id}`} className="relative">
