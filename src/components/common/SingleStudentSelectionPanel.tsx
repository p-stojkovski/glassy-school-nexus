...
const SingleStudentSelectionPanel: React.FC<SingleStudentSelectionPanelProps> = ({
  ...
  const hasOngoingObligations = (studentId: string): boolean => {
    const studentObligations = obligations.filter(obl => obl.studentId === studentId);
  ...
  const getObligationSummary = (studentId: string) => {
    const studentObligations = obligations.filter(obl => obl.studentId === studentId);
    const totalObligations = studentObligations.reduce((sum, obl) => sum + obl.amount, 0);
    ...
    const statusCounts = {
      pending: studentObligations.filter(obl => obl.status === 'pending').length,
      partial: studentObligations.filter(obl => obl.status === 'partial').length,
      overdue: studentObligations.filter(obl => obl.status === 'overdue').length,
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (filterOngoingObligationsOnly) {
      filtered = filtered.filter(student => hasOngoingObligations(student.id));
    }
  ...
  }, [students, searchQuery, statusFilter, gradeFilter, filterOngoingObligationsOnly, hasOngoingObligations]);
            ...
            <div className="bg-white/10 rounded-lg p-3 border border-white/20">
              <div className="flex items-center justify-between">
                <div>                  <p className="text-white font-medium">
                  ...
                  <p className="text-white/70 text-sm">ID: {selectedStudent.id}</p>                  {filterOngoingObligationsOnly && (
                    <div className="mt-2">
                      {(() => {
                        const summary = getObligationSummary(selectedStudent.id);
                        return (
                          <div className="text-xs text-white/80">
                            <p>Outstanding: ${summary.remaining.toFixed(2)}</p>
                            <p>
                              {summary.statusCounts.pending > 0 && `${summary.statusCounts.pending} pending, `}
                              {summary.statusCounts.partial > 0 && `${summary.statusCounts.partial} partial, `}
                              {summary.statusCounts.overdue > 0 && `${summary.statusCounts.overdue} overdue`}
                            </p>
                          </div>
                        );
                      })()}
                ...
                <Button variant="ghost" size="sm" onClick={clearSelection} className="text-white/70 hover:text-white hover:bg-white/20">
                  <X className="w-4 h-4" />
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {filteredStudents.length === 0 ? (
                <div className="text-center py-8">
                  <User className="w-12 h-12 text-white/40 mx-auto mb-2" />                  <p className="text-white/60">
                    {filterOngoingObligationsOnly ? 'No students with ongoing obligations found' : 'No students found'}
                  </p>
                </div>
              ) : (
                filteredStudents.map((student) => {
                  const isSelected = selectedStudentId === student.id;
                  const classData = student.classId ? classes.find(c => c.id === student.classId) : null;
                  const obligationSummary = filterOngoingObligationsOnly ? getObligationSummary(student.id) : null;
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2">                            <h3 className="text-white font-medium">
                              {student.name}
                            </h3>
                            <span className={cn(
                              "px-2 py-1 rounded-full text-xs",
                              student.status === 'active' ? "bg-green-600/30 text-green-300" :
                              student.status === 'inactive' ? "bg-red-600/30 text-red-300" :
                          ...
                          {classData && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-xs text-white/60">{classData.name}</span>
                            </div>
                          )}
                          {obligationSummary && obligationSummary.remaining > 0 && (
                            <div className="mt-2 text-xs">
                              <p className="text-orange-300">
                                Outstanding: ${obligationSummary.remaining.toFixed(2)}
                              </p>
                              <div className="flex gap-2 mt-1">
                                {obligationSummary.statusCounts.pending > 0 && (
                                  <span className="bg-yellow-600/30 text-yellow-300 px-1 py-0.5 rounded">
                                    {obligationSummary.statusCounts.pending} pending
                                  </span>
                                )}
                                {obligationSummary.statusCounts.partial > 0 && (
                                  <span className="bg-blue-600/30 text-blue-300 px-1 py-0.5 rounded">
                                    {obligationSummary.statusCounts.partial} partial
                                  </span>
                                )}
                                {obligationSummary.statusCounts.overdue > 0 && (
                                  <span className="bg-red-600/30 text-red-300 px-1 py-0.5 rounded">
                                    {obligationSummary.statusCounts.overdue} overdue
                                  </span>
                                )}
                              </div>
                            </div>
