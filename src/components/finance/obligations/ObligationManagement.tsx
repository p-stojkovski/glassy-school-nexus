...
const ObligationManagement: React.FC = () => {  const [currentTab, setCurrentTab] = useState('view');
  ...
  return (    
    <Card className="p-6 bg-white/10 backdrop-blur-sm border-white/20">
      <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">        <TabsList className="flex w-full mb-6 bg-white/20 gap-2">
          ...
          <TabsTrigger value="add" className="flex-1 text-white">Add Obligations</TabsTrigger>
        </TabsList>        <TabsContent value="view" className="space-y-4">
          <ObligationTable />
        </TabsContent>
