...
// Simple in-memory cache to avoid refetching summaries when tab re-renders
const summariesCache = new Map<string, { version: number; summaries: StudentLessonSummary[] }>();
...
const StudentProgressTable: React.FC<StudentProgressTableProps> = ({
  ...
  // Fetch summary data on mount or when version changes
  useEffect(() => {
    let cancelled = false;
    ...
    const fetchSummaries = async () => {
      ...
      const existing = summariesInFlight.get(inFlightKey);
      if (existing) {
        setLoading(true);
        setError(null);
        try {
      ...
      try {
        ...
        const request = classApiService.getClassStudentsSummary(classId);
        ...
        summariesInFlight.delete(`${classId}:${dataVersion}`);
