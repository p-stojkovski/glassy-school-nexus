// Lesson display components
/** @deprecated Use LessonTimeline instead. LessonCalendar will be removed in a future version. */
export { default as LessonCalendar } from './LessonCalendar';
export { default as LessonTimeline } from './LessonTimeline';
export { default as LessonStatusBadge } from './LessonStatusBadge';
export { default as LessonsSummaryStrip } from './LessonsSummaryStrip';
export { default as LessonsEnhancedFilters } from './LessonsEnhancedFilters';
export { default as LessonStatusChips } from './LessonStatusChips';
export { default as LessonRowActionsMenu } from './LessonRowActionsMenu';
export { default as LessonEditHistorySidebar } from './LessonEditHistorySidebar';

// Modal components
export { default as CreateLessonSidebar } from './modals/CreateLessonSidebar';
export { default as QuickCancelLessonModal } from './modals/QuickCancelLessonModal';
export { default as QuickConductLessonModal } from './modals/QuickConductLessonModal';
export { default as LessonDetailModal } from './modals/LessonDetailModal';
export { default as AcademicLessonGenerationModal } from './modals/AcademicLessonGenerationModal';

