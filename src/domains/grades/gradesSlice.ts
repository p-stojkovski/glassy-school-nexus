...
export type AssessmentType = 'Homework' | 'Test' | 'Quiz' | 'Project' | 'Participation' | string;
...
export const gradesSlice = createSlice({
  ...
  reducers: {
    ...
    deleteAssessment: (state, action: PayloadAction<string>) => {
      ...
      state.grades = state.grades.filter(g => g.assessmentId !== action.payload);
      localStorage.setItem('assessments', JSON.stringify(state.assessments));
...
// Selectors
export const selectAllAssessments = (state: RootState) => state.grades.assessments;
...
export const selectSelectedAssessmentId = (state: RootState) => state.grades.selectedAssessmentId;
export const selectSelectedClassId = (state: RootState) => state.grades.selectedClassId;

export const selectAssessmentsByClassId = (state: RootState, classId: string) => 
  state.grades.assessments.filter(assessment => assessment.classId === classId);

export const selectGradesByAssessmentId = (state: RootState, assessmentId: string) => 
