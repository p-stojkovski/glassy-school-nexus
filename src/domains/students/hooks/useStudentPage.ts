...
export interface UseStudentPageResult {
  ...
  gradeAssessments: { assessmentId: string; assessment?: { id: string; name: string } }[];
...
export const useStudentPage = (): UseStudentPageResult => {
  ...
  const obligations = useAppSelector((state: RootState) =>
    studentId && studentId !== 'new' ? selectObligationsByStudentId(state, studentId) : []
  );
  
  const payments = useAppSelector((state: RootState) =>
    studentId && studentId !== 'new' ? selectPaymentsByStudentId(state, studentId) : []
  );
  ...
  const attendanceRecords = useAppSelector((state: RootState) =>
    currentClassId ? selectAttendanceByClassId(state, currentClassId) : []
  );

  const grades = useAppSelector((state: RootState) =>
    studentId && studentId !== 'new' ? selectGradesByStudentId(state, studentId) : []
  );
