...
const attendanceSlice = createSlice({
  ...
  reducers: {
    ...
    setAttendanceRecords: (
      state,
      action: PayloadAction<BasicAttendanceRecord[]>
    ) => {
    ...
    updateStudentAttendance: (
      ...
      if (recordIndex !== -1) {
        const studentIndex = state.attendanceRecords[
          recordIndex
        ].studentRecords.findIndex((sr) => sr.studentId === studentId);
        ...
        if (studentIndex !== -1) {
          ...
          if (state.currentRecord?.id === recordId) {
            const currentStudentIndex =
              state.currentRecord.studentRecords.findIndex(
                (sr) => sr.studentId === studentId
              );
            if (currentStudentIndex !== -1) {
              state.currentRecord.studentRecords[currentStudentIndex].status =
                status;
              if (notes !== undefined) {
...
export const selectAttendanceByDate = (state: RootState, date: string) =>
  state.attendance.attendanceRecords.filter(
    (record) => record.sessionDate === date
  );
