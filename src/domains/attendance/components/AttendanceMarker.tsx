...
const AttendanceMarker: React.FC<AttendanceMarkerProps> = ({ classId, date }) => {
  ...
  useEffect(() => {
    if (classId && date) {
      ...
      if (existingRecord) {
        ...
        const studentData: Record<string, { status: StatusType; notes: string }> = {};
        ...
        if (selectedClass) {
          ...
          const initialData: Record<string, { status: StatusType; notes: string }> = {};
  ...
  if (!classId || classId === 'all-classes') {
    return (
      <GlassCard className="p-6 text-white text-center py-12">
        <h3 className="text-xl">Please select a specific class to mark attendance</h3>
      </GlassCard>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    const unmarkedStudents = classStudents.filter(student => 
  ...
  const confirmSubmit = () => {
    ...
    const studentRecords: StudentAttendance[] = Object.entries(attendanceData).map(
      ([studentId, data]) => {
    ...
    if (isEditing && currentRecordId) {
      ...
      const updatedRecord: AttendanceRecord = {
        ...
        createdAt: attendanceRecords.find(r => r.id === currentRecordId)?.createdAt || new Date().toISOString()
      };
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-medium text-white">
              {isEditing ? 'Edit Attendance' : 'Mark Attendance'}: {selectedClass.name}
            </h3>
          ...
          <Table className="text-white">
            <TableHeader>
              <TableRow className="hover:bg-white/5 border-white/20">
                ...
                <TableHead className="text-white/70 w-[200px]">Status</TableHead>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-2">
                        <div 
                          className={`w-4 h-4 rounded-full ${
                            attendanceData[student.id]?.status === 'present' ? 'bg-green-500' : 'bg-white/20'
                          }`}
                      ...
                      <div className="flex items-center space-x-2">
                        <div 
                          className={`w-4 h-4 rounded-full ${
                            attendanceData[student.id]?.status === 'absent' ? 'bg-red-500' : 'bg-white/20'
                          }`}
                      ...
                      <div className="flex items-center space-x-2">
                        <div 
                          className={`w-4 h-4 rounded-full ${
                            attendanceData[student.id]?.status === 'late' ? 'bg-yellow-500' : 'bg-white/20'
                          }`}
