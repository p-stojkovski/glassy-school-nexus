...
const FinancialDashboard: React.FC = () => {
  ...
  const obligationsByStatus = obligations.reduce(
    (acc, obligation) => {
  ...
  const statusData = [
    {
      name: 'Paid',
      value: obligationsByStatus.paid?.amount || 0,
      count: obligationsByStatus.paid?.count || 0,
      color: '#10B981',
    },
    {
      name: 'Partial',
      value: obligationsByStatus.partial?.amount || 0,
      count: obligationsByStatus.partial?.count || 0,
      color: '#3B82F6',
    },
    {
      name: 'Pending',
      value: obligationsByStatus.pending?.amount || 0,
      count: obligationsByStatus.pending?.count || 0,
      color: '#D97706',
    },
    {
      name: 'Overdue',
      value: obligationsByStatus.overdue?.amount || 0,
      count: obligationsByStatus.overdue?.count || 0,
      color: '#EF4444',
    },
  ];
  ...
  const upcomingObligations = obligations
    .filter(
      (obligation) =>
        obligation.status !== 'paid' && obligation.dueDate >= today
    )
    .sort(
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="bg-white/5 backdrop-blur-sm border-white/15">
          ...
          <CardContent>
            ...
            <p className="text-xs text-white/70">
              Across {obligations.length} obligations
            </p>
          </CardContent>
        </Card>
        <Card className="bg-white/5 backdrop-blur-sm border-white/15">
        ...
        <Card className="bg-white/5 backdrop-blur-sm border-white/10">
          ...
          <CardContent>
            ...
            <Progress
              ...
              className="mt-2 bg-white/30 [&>div]:bg-cyan-400"
            />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {' '}
        <Card className="col-span-1 bg-white/10 backdrop-blur-sm border-white/20">
          <CardHeader>
        ...
        <Card className="col-span-1 bg-white/5 backdrop-blur-sm border-white/10">
          <CardHeader>
      ...
      {/* Recent Transactions and Upcoming Due */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {' '}
        <Card className="col-span-1 bg-white/5 backdrop-blur-sm border-white/10">
          <CardHeader>
